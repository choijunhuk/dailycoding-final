...
router.get('/ai-status', auth, adminOnly, async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Failed to load AI status.' });
  }
});
...
router.get('/flagged-submissions', auth, adminOnly, async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Failed to load flagged submissions.' });
  }
});
...
router.post('/problems/:id/troubleshooting', auth, adminOnly, async (req, res) => {
  ...
  try {
    ...
    if (!TroubleshootingProblem.isTroubleshootingType(problem.problemType)) {
      return res.status(400).json({ message: 'This problem is not a troubleshooting type.' });
    }
    ...
    return res.status(status).json({ message: status < 500 ? err.message : 'Failed to save troubleshooting configuration.' });
  }
});
...
router.get('/battle-settings', auth, adminOnly, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to load battle settings.' });
  }
});
...
router.put('/battle-settings', auth, adminOnly, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to save battle settings.' });
  }
});
...
router.delete('/content/bulk', auth, adminOnly, async (req, res) => {
  ...
  if (!['dump', 'community', 'all'].includes(target)) {
    return res.status(400).json({ message: 'target must be one of: dump | community | all.' });
  }
  ...
  try {
    ...
    res.json({ message: `Data older than ${days} days deleted successfully.`, deleted: results });
  } catch (err) {
...
router.patch('/reports/blind', auth, adminOnly, async (req, res) => {
  ...
  try {
    ...
    res.json({ message: `${blind ? 'Blinded' : 'Unblinded'} successfully.` });
  } catch (err) {
