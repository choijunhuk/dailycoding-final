...
test('profile background defaults do not expose the old dark duplicate', () => {
  ...
  assert.equal(bySlug.get(DEFAULT_PROFILE_BACKGROUND_SLUG)?.name, 'Default Slate');
  ...
  assert.equal(PROFILE_BACKGROUND_SEEDS.some((item) => item.name === '기본 다크' || item.name === 'Default Dark'), false);
});
