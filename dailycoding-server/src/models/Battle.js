...
const FILL_BLANK_PROBLEMS = [
  {
    ...
    title: 'Fill in the Blank: Recursive Fibonacci',
    desc: 'Fill in the blanks in this recursive function that computes the Fibonacci sequence.',
    codeTemplate: 'def fib(n):\n    if n <= ___1___:\n        return n\n    return fib(n - ___2___) + fib(n - ___3___)',
    blanks: ['1', '1', '2'],
    answerGroups: [[0], [1, 2]],
    hint: 'Since fib(0)=0 and fib(1)=1, just return n when n is small.',
  },
  {
    id: 'fb_2', type: 'fill-blank',
    preferredLanguage: 'python',
    title: 'Fill in the Blank: Binary Search',
    desc: 'Fill in the blanks in this binary search code.',
    codeTemplate: 'def binary_search(arr, target):\n    lo, hi = ___1___, len(arr) - 1\n    while lo <= hi:\n        mid = (lo + hi) // ___2___\n        if arr[mid] == target: return mid\n        elif arr[mid] < target: lo = ___3___\n        else: hi = mid - 1\n    return -1',
    blanks: ['0', '2', 'mid + 1'],
    hint: 'Calculate mid while halving the search range each iteration.',
  },
];
...
const BUG_FIX_PROBLEMS = [
  {
    ...
    preferredLanguage: 'python',
    title: 'Bug Fix: Find Maximum Value',
    desc: 'This code finds the maximum value in an array. Find the bug and enter the corrected line.',
    buggyCode: 'def find_max(arr):\n    max_val = 0      # bug!\n    for x in arr:\n        if x > max_val:\n            max_val = x\n    return max_val',
    correctAnswerKeyword: 'arr[0]',
    explanation: 'Initializing max_val = 0 causes incorrect behavior when all elements are negative. It should be initialized to arr[0].',
    hint: 'Try testing with an all-negative array like [-5, -3, -1].',
  },
  {
    id: 'bf_2', type: 'bug-fix',
    preferredLanguage: 'python',
    title: 'Bug Fix: Bubble Sort IndexError',
    desc: 'This bubble sort raises an IndexError. Find and fix the buggy line.',
    buggyCode: 'def bubble_sort(arr):\n    n = len(arr)\n    for i in range(n):\n        for j in range(n - i):  # bug!\n            if arr[j] > arr[j + 1]:\n                arr[j], arr[j + 1] = arr[j + 1], arr[j]\n    return arr',
    correctAnswerKeyword: 'n - i - 1',
    explanation: 'range(n - i) causes j+1 to exceed the array bounds. It should be range(n - i - 1).',
    hint: 'Adjust the upper bound of range so that j+1 does not go out of array bounds.',
  },
];
