import { getProblemTypeLabel, getTagLabel as getDisplayTagLabel } from '../utils/labelMaps.js';
...
export function getProblemTypeMeta(problemType = 'coding', lang = 'en') {
  ...
  const key = source[problemType] ? problemType : 'coding';
  const meta = source[key];
  if (key !== problemType) return meta;
  return {
    ...meta,
    label: getProblemTypeLabel(key, lang),
    short: getProblemTypeLabel(key, lang, { short: true }),
  };
}
...
export function getTagLabelLang(tag, lang) {
  ...
  return getDisplayTagLabel(tag, lang);
}
