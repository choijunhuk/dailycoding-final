...
export const TEMPLATES = {
  python: [
    { name: 'Basic I/O', code: 'import sys\ninput = sys.stdin.readline\n\nn = int(input())\narr = list(map(int, input().split()))\n\n# solution\nprint(result)\n' },
    { name: 'Multi-line Input', code: 'import sys\ninput = sys.stdin.readline\n\nn = int(input())\nfor _ in range(n):\n    line = input().strip()\n    # process\n' },
    { name: 'BFS Traversal', code: 'from collections import deque\n\ndef bfs(start):\n    visited = set([start])\n    q = deque([start])\n    while q:\n        cur = q.popleft()\n        for nxt in graph[cur]:\n            if nxt not in visited:\n                visited.add(nxt)\n                q.append(nxt)\n' },
    { name: 'DP Basic', code: 'n = int(input())\ndp = [0] * (n + 1)\n\n# base cases\ndp[0] = 0\ndp[1] = 1\n\nfor i in range(2, n + 1):\n    dp[i] = dp[i-1] + dp[i-2]\n\nprint(dp[n])\n' },
  ],
  javascript: [
    { name: 'Basic I/O (Node)', code: 'const input = require("fs").readFileSync("/dev/stdin","utf8").trim().split("\\n");\nconst n = Number(input[0]);\nconst arr = input[1].split(" ").map(Number);\n\n// solution\nconsole.log(result);\n' },
    { name: 'BFS Traversal', code: 'function bfs(start, graph) {\n  const visited = new Set([start]);\n  const q = [start];\n  while (q.length) {\n    const cur = q.shift();\n    for (const nxt of graph[cur]) {\n      if (!visited.has(nxt)) {\n        visited.add(nxt);\n        q.push(nxt);\n      }\n    }\n  }\n}\n' },
  ],
  cpp: [
    { name: 'Basic I/O', code: '#include <bits/stdc++.h>\nusing namespace std;\n\nint main() {\n    ios::sync_with_stdio(false);\n    cin.tie(nullptr);\n    \n    int n;\n    cin >> n;\n    \n    // solution\n    \n    cout << result << "\\n";\n    return 0;\n}\n' },
    { name: 'BFS Traversal', code: '#include <bits/stdc++.h>\nusing namespace std;\n\nvector<int> graph[100001];\nbool visited[100001];\n\nvoid bfs(int start) {\n    queue<int> q;\n    q.push(start);\n    visited[start] = true;\n    while (!q.empty()) {\n        int cur = q.front(); q.pop();\n        for (int nxt : graph[cur]) {\n          if (!visited[nxt]) {\n            visited[nxt] = true;\n            q.push(nxt);\n          }\n        }\n    }\n}\n' },
  ],
  java: [
    { name: 'Basic I/O', code: 'import java.io.*;\nimport java.util.*;\n\npublic class Main {\n    public static void main(String[] args) throws IOException {\n        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));\n        int n = Integer.parseInt(br.readLine());\n        StringTokenizer st = new StringTokenizer(br.readLine());\n        \n        // solution\n        \n        System.out.println(result);\n    }\n}\n' },
  ],
  c: [
    { name: 'Basic I/O', code: '#include <stdio.h>\n\nint main() {\n    int n;\n    scanf("%d", &n);\n    \n    // solution\n    \n    printf("%d\\n", result);\n    return 0;\n}\n' },
  ],
};
